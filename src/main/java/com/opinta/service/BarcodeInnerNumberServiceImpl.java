...
public class BarcodeInnerNumberServiceImpl implements BarcodeInnerNumberService {
    ...
    public BarcodeInnerNumberDto save(long postcodeId,
        ...
        PostcodePool postcodePool = postcodePoolDao.getById(postcodeId);
        if (postcodePool == null) {
            log.debug("Can't add barcodeInnerNumberDto to postcodePool. "
                    + "PostCodePool {} doesn't exist", postcodeId);
            return null;
        }
        BarcodeInnerNumber barcodeInnerNumber = barcodeInnerNumberMapper
                .toEntity(barcodeInnerNumberDto);
        ...
        log.info("Adding barcodeInnerNumber {} to postcodePool {}",
                barcodeInnerNumber, postcodePool);
    ...
    public BarcodeInnerNumberDto update(long id, BarcodeInnerNumberDto barcodeInnerNumberDto) {
        ...
        try {
            ...
            log.error("Can't get properties from object to updatable object "
                    + "for barcodeInnerNumber", e);
        }
        target.setId(id);
