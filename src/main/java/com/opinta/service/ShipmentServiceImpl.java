...
public class ShipmentServiceImpl implements ShipmentService {
    ...
    private BigDecimal calculatePrice(Shipment shipment) {
        List<Parcel> parcelList = shipment.getParcelList();
        BigDecimal resultPrice = BigDecimal.ZERO;
        if (parcelList == null){
            return resultPrice;
        }
        for (Parcel parcel:parcelList) {
            parcelService.setPriceToParcel(shipment, parcel);
            BigDecimal priceOfOneParcel = parcel.getPrice();
            resultPrice = resultPrice.add(priceOfOneParcel);
        }
        return resultPrice;
    }
    ...
    public BigDecimal getDeclaredPrice (Shipment shipment){
        ...
        List<Parcel> parcelList = shipment.getParcelList();
        for (Parcel element:parcelList) {
            declaredPrice.add(element.getDeclaredPrice());
        }
