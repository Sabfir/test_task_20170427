...
public class AddressController {
    ...
    public ResponseEntity<?> getAddress(@PathVariable("id") long id) {
        ...
        if (addressDto == null) {
            return new ResponseEntity<>(format("Get: No Address found for ID %d", id), NOT_FOUND);
        }
    ...
    public ResponseEntity<?> updateAddress(@PathVariable long id, @RequestBody AddressDto addressDto) {
        ...
        if (addressDto == null) {
            return new ResponseEntity<>(format("Update: No Address found for ID %d", id), NOT_FOUND);
        }
        return new ResponseEntity<>(addressDto, OK);
    }
    ...
    public ResponseEntity<?> deleteAddress(@PathVariable long id) {
        if (!addressService.delete(id)) {
            return new ResponseEntity<>(format("Delete: No Address found for ID %d", id), NOT_FOUND);
        }
