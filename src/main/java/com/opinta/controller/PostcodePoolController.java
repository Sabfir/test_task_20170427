...
public class PostcodePoolController {
    public static final String POSTCODE_POOL_NOT_FOUND = "No PostcodePool found for ID %d";
    public static final String POSTCODE_POOL_NOT_EXIST = "PostcodePool %d doesn't exist";
    public static final String BARCODE_INNER_NUMBER_NOT_FOUND = "No barcodeInnerNumber found"
            + " for ID %d";
    ...
    public ResponseEntity<?> getPostcodePool(@PathVariable("id") long id) {
        ...
        if (postcodePoolDto == null) {
            return new ResponseEntity<>(format(POSTCODE_POOL_NOT_FOUND, id), NOT_FOUND);
        }
    ...
    @PutMapping("{id}")
    public ResponseEntity<?> updatePostcodePool(
            @PathVariable long id, @RequestBody PostcodePoolDto postcodePoolDto) {
    ...
    public ResponseEntity<?> getBarcodeInnerNumbers(@PathVariable long postcodeId) {
        List<BarcodeInnerNumberDto> barcodeInnerNumberDtos = barcodeInnerNumberService
                .getAll(postcodeId);
        if (barcodeInnerNumberDtos == null) {
            return new ResponseEntity<>(format(POSTCODE_POOL_NOT_EXIST, postcodeId), NOT_FOUND);
        }
    ...
    public ResponseEntity<?> getBarcodeInnerNumber(@PathVariable("id") long id) {
        ...
        if (barcodeInnerNumberDto == null) {
            return new ResponseEntity<>(format(BARCODE_INNER_NUMBER_NOT_FOUND, id), NOT_FOUND);
        }
    ...
    public ResponseEntity<?> createBarcodeInnerNumber(
            @PathVariable("postcodeId") long postcodeId,
