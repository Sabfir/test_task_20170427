...
public class ShipmentTrackingDetailController {
	private static final String SHIPMENT_TRACKING_ERROR = "No ShipmentTrackingDetail found for ID %d";
	...
	public ResponseEntity<?> getShipmentTrackingDetail(@PathVariable("id") long id) {
		...
		if (shipmentTrackingDetailDto == null) {
			return new ResponseEntity<>(format(SHIPMENT_TRACKING_ERROR, id), NOT_FOUND);
		}
